...
export function defaultCurrency() {
    if ( typeof window !== 'undefined' && window.dono?.default_currency ) {
        return String( window.dono.default_currency ).toUpperCase();
    }
    return 'USD';
}
...
// Explicit override channel for surfaces with no window.dono (the public
...
export function numberFormat() {
    ...
    if ( typeof window !== 'undefined' && window.dono?.number_format ) {
        return { ...DEFAULT_NUMBER_FORMAT, ...window.dono.number_format };
    }
...
export function timeAgo( iso ) {
    ...
    if ( diff < 3600 )    return sprintf( __( '%dm ago', 'dono-fundraising-platform' ),  Math.floor( diff / 60 ) );
    /* translators: %d: number of hours */
    if ( diff < 86400 )   return sprintf( __( '%dh ago', 'dono-fundraising-platform' ),  Math.floor( diff / 3600 ) );
    /* translators: %d: number of days */
    if ( diff < 604800 )  return sprintf( __( '%dd ago', 'dono-fundraising-platform' ),  Math.floor( diff / 86400 ) );
